<div class="well">
    ...
    VALUES ({$_GET['id']},'{$comment_name}','{$comment_email}','{$comment_content}','{$comment_date}','unapproved')";
        ...
        if (!$result) {
            ...
            $query = "UPDATE posts SET post_comment_count = post_comment_count + 1 WHERE post_id = {$_GET['id']}";
            # tip: cannot use += in sql query but you can use x = x + 1
            $result = mysqli_query($conn,$query);
            if(!$result){
                echo "problem to update post comment count";
            }
